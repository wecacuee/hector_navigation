...
cv::Mat convertToGTSize(const cv::Mat &original_map, const cv::Size groundtruth_size, unsigned char padValue)
{
  ...
  if (diff_size_rows > 0)
  {
    cv::Range row_range((int) std::floor(diff_size_rows / 2.0),
                        original_map.rows - (int) std::ceil(diff_size_rows / 2.0));
  ...
  if (diff_size_cols > 0)
  {
    cv::Range col_range((int) std::floor(diff_size_cols / 2.0),
                        original_map.cols - (int) std::ceil(diff_size_cols / 2.0));
                                              ...
                                              const boost::shared_ptr<hector_exploration_planner::CustomCostmap2DROS> &costmap_2d_ros)
{
  ...
  for (const auto &world_point: world_points)
  {
    ...
    // y is negated because y is pointing downwards in opencv coords but upwards in costmap coords
    map_y = static_cast<unsigned int>(-groundtruth_position_vector.getY() / resolution +
                                      costmap->getSizeInCellsY() / 2);
...
void preprocessFrontierImg(cv::Mat &frontier_img_in, cv::Mat &frontier_img_out)
{
  cv::Mat element = cv::getStructuringElement(cv::MORPH_RECT,
                                              cv::Size(2 * FRONTIER_OPENING_KERNEL + 1,
                                                       2 * FRONTIER_OPENING_KERNEL + 1),
                    ...
                    cv::Mat &output_img)
{
  ...
  cv::Size costmap_center(std::round(costmap.getSizeInCellsX() / (float) 2),
                          std::round(costmap.getSizeInCellsY() / (float) 2));
...
//      cv::Mat rawCostMap_resized_clipped = frontier_analysis::convertToGroundtruthSize(rawCostMap_resized, GTSize);
  cv::Mat costMap_resize_clipped = frontier_analysis::convertToGTSizeFillUnknown(costMap_resize, desired_size);
  assert(costMap_resize_clipped.size() == desired_size);
  std::vector<std::vector<Pose2D>> frontiers_resize_clipped;
  frontier_analysis::convertToGroundTruthSize(frontiers_resize, frontiers_resize_clipped, currentSize, desired_size);
  Pose2D robotPose_resize_clipped = frontier_analysis::convertToGroundTruthSize(robotPose_resize, currentSize,
                                                                                desired_size);
  ...
  cv::Mat boundingBoxImg = frontier_analysis::generateBoundingBoxImage(boundingBoxes, desired_size);

  costmap_output = costMap_resize_clipped;
  bounding_box_output = boundingBoxes;
  frontier_clusters = frontiers_resize_clipped;
}
